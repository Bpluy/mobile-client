...
public partial class RegisterPage : ContentPage
{
    ...
    private async void Register_Clicked(object sender, EventArgs e)
    {
        if (isFilled)
        {
            ...
            {
                await DisplayAlert("Неверное подтверждение пароля", "Пароли не совпадают. Повторите попытку.", "OK");
            }
            else
            {
                var port = 11333;
                ...
                try
                {
                    ...
                    if (isSuccess)
                    {
                        await DisplayAlert("Успешная регистрация", "Выполните вход по введённым данным", "OK");
                        await Navigation.PopAsync();
                    }
                    else
                    {
                        await DisplayAlert("Ошибка", "Данный логин занят.", "OK");
                    }
                }
